  ...
  try {
    ...
    if (!user) {
      ...
      user = new User({
        ...
        name,
        email,
        phone: '', // You can prompt the user to add this later
...
const registerUser = async (req, res) => {
  ...
  try {
    ...
    // Create the user without saving the token to the database
    const user = await User.create({ name, email, phone, password: hashedPassword });

    // Generate a token
    const token = jwt.sign({ id: user._id }, process.env.JWT_SECRET, { expiresIn: '1h' });
    // Send the token to the client (e.g., in a cookie or response body)
    res.cookie('auth_token', token, { httpOnly: true, secure: true });
...
module.exports = { registerUser, loginUser, getCurrentUser, sendOtp, verifyOtp, resetPass, updateUser, firebaseLogin };
